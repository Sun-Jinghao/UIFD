...
class Flashes(BaseDataset):
    def __init__(self, config, device):
        ...
        # Mixed aachen_transfer
        # train_dir_aachen = Path(root_dir, 'city_flashes') #加进去路径
        # train_sequence_paths_aachen = [path for path in train_dir_aachen.iterdir()]
        # for seq_path in train_sequence_paths_aachen:
        #     images_path = [str(x) for x in seq_path.iterdir()]
        #     self._paths['train'] += [(images_path[i], images_path[i + 1])
        #                              for i in range(len(images_path) - 1)]
