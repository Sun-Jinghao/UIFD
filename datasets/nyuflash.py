...
class Nyuflash(BaseDataset):
    def __init__(self, config, device):
        ...
        for seq_path in train_sequence_paths:
            ...
            self._paths['train'] += [(images_path[i], images_path[j])
        ...
        for seq_path in val_sequence_paths:
            ...
            self._paths['val'] += [(images_path[i], images_path[j])
                                     for i in range(len(images_path))
                                         for j in range(len(images_path))
                                            if i!=j]
        ...
        for seq_path in test_sequence_paths:
            ...
            self._paths['test'] += [(images_path[i], images_path[j])
                                     for i in range(len(images_path))
                                         for j in range(len(images_path))
                                            if i!=j]
