...
class BaseModel(metaclass=ABCMeta):
    ...
    def __init__(self, dataset, config, device):
        ...
        self._net = self._net.cuda() #to(device)
        self._solver = optim.Adam(self._net.parameters(), lr=self._config['learning_rate'])
        #self._solver = optim.AdamW(self._net.parameters(), lr=self._config['learning_rate'])
        self._it = 0
        self._epoch = 0

    def _to_dict(self, d, device):
        """ Send all the values of a dict of Tensor to a specific device. """
        return {k: v.cuda() if isinstance(v, torch.Tensor) else v #to(device)
                for k, v in d.items()}
