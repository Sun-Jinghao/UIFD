...
from .layers import ResBlock_do_fft_bench
...
class HrNet(nn.Module):
    ...
    def __init__(self, output_dim=128, input_chan=3):
        ...
        self.conv01 = nn.Sequential(DOConv2d(3, 32, 3, padding=1, stride=1, bias=False, groups=1))
        self.conv32 = nn.Sequential(DOConv2d(32, 32, 3, padding=1, stride=1, bias=False, groups=1))
        self.conv64 = nn.Sequential(DOConv2d(64, 64, 3, padding=1, stride=1, bias=False, groups=1))
    ...
    def forward(self, x):
        ...
        feat0 = self.stage0_layer1(self.conv32(feat0))
        feat1 = self.stage1_layer0(self.cbam1(self.conv32(feat1)))
        feat1_up = F.interpolate(self.preupsoft0(feat1), size=[h0, w0], mode='bilinear', align_corners=True)
        feat0_down = self.x1down(feat0)
        feat0 = self.stage0_layer2(self.conv64(torch.cat((feat0, feat1_up), dim=1)))
        feat0 = self.conv22(feat0)
        feat1 = self.stage1_layer1(self.cbam2(self.conv64(torch.cat((feat1, feat0_down), dim=1))))
        ...
        feat0 = self.stage0_layer3(self.conv64(torch.cat((feat0, feat1_up), dim=1)))
        feat1 = self.stage1_layer2(self.cbam2(self.conv64(torch.cat((feat1, feat0_down), dim=1))))
        del feat1_up, feat0_down
        feat2 = self.stage2_layer0(self.cbam2(self.conv64(torch.cat((feat0_down_down, feat1_down), dim=1))))
