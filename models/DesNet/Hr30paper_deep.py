...
class HrNet(nn.Module):
    ...
    def __init__(self, output_dim=128, input_chan=3):
        ...
        self.preupsoft2 = nn.Conv2d(64, 64, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        ##04
        self.stage0_layer4 = nn.Sequential(  # 3,16
        ...
        ##13
        self.stage1_layer3 = nn.Sequential(  # 3,16
            nn.Conv2d(192, 192, kernel_size=3, stride=1, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(192, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.InstanceNorm2d(128, affine=False)
        ...
        #21
        self.stage2_layer1 = nn.Sequential(  # 3,16
            nn.Conv2d(192, 192, kernel_size=3, stride=1, padding=1, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(192, 128, kernel_size=3, stride=1, padding=1, bias=False),
            nn.InstanceNorm2d(128, affine=False)
        ...
        self.desc_head = nn.Sequential(  # 3,16
            ...
            nn.Conv2d(384, output_dim, kernel_size=3, stride=1, padding=1, bias=False)
        )
    ...
    def forward(self, x):
        ...
        feat1up = F.interpolate(self.preupsoft2(feat1), size=[h0, w0], mode='bilinear', align_corners=True)
        feat2up1 = F.interpolate(self.preupsoft2(feat2), size=[120, 160], mode='bilinear', align_corners=True)
        feat2up = F.interpolate(self.preupsoft2(feat2), size=[h0, w0], mode='bilinear', align_corners=True)

        # print(feat2up1.shape)
        # print(feat2up.shape)

        feat04 = self.stage0_layer4(torch.cat((feat0,feat1up,feat2up), dim=1))
        
        feat0down = self.x1down(feat0)
        feat1down = self.x1down(feat1)
        feat0down2 = self.x2down(feat0)

        # print(feat0down.shape)
        # print(feat1.shape)
        # print(feat2up1.shape)
        feat13 = self.stage1_layer3(torch.cat((feat0down,feat1,feat2up1), dim=1))

        feat21 = self.stage2_layer1(torch.cat((feat0down2,feat2,feat2), dim=1))

        del feat0_down_down, feat1_down
        feat0 = self.x2down(feat04)
        feat1 = self.x1down(feat13)
        x = self.desc_head(torch.cat((feat0, feat1, feat21), dim=1)) # scale = 0.25x
        outputs = {
            'raw_descs_fine': x
        }
        return outputs
