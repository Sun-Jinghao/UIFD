...
class Hr(nn.Module):
    ...
    def _forw_impl(self, x):
        ...
        x4_down1 = self.down(x4)
        x4_down2 = self.down(x4_down1)

        x41_down1 = self.down(x41)
        x410_up = self.up3(x410)
        x51 = self.layer5(torch.cat((x410_up,(torch.cat(x4_down1,x41),1)),1))

        c = torcn.cat((x4_down2,x41_down1),)
        c = torch.cat((x410,c),1)
